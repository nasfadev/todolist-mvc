<main id="main-content" class="sm:flex flex-wrap sm:divide-none divide-y  sm:*:basis-1/2 lg:*:basis-1/3 *:px-4  *:py-2 sm:*:mt-0 sm:mx-14 sm:mt-24 mt-16">
    <?php for ($i = 0; $i < count($data); $i++) : ?>
        <?php $user_data = $this->model('User')->getOne($data[$i]["user_id"]);  ?>
        <article class="group sm:rounded-lg duration-200 hover:bg-slate-50 flex flex-col space-y-2 cursor-pointer">
            <div class="flex items-center justify-between">
                ...
                <div class="flex flex-col grow">
                    <div class="flex *:px-2">
                        <h3 class="font-medium grow"><?php echo $data[$i]["title"] ?></h3>
            ...
            <div class="mb-0 flex justify-around items-center text-slate-600 h-7 *:h-full *:w-14 *:justify-center">
                <div class="hover:text-blue-700 flex group/b-like items-center space-x-3">
                    ...
                    <span class="text-sm z-10"><?php echo $data[$i]["comment_count"] ?></span>
                </div>
                <div id="like-btn" class="select-none hover:text-pink-600 cursor-pointer flex group/b-like items-center space-x-3">
                    ...
                    <span class="text-sm z-10 pointer-events-none"><?php echo $data[$i]["like_count"] ?></span>
                </div>
